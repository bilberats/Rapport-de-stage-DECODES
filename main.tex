...
\begin{document}
	...
	\section{Environnement et organisation du travail}
	...
	Télécom SudParis est implantée sur deux sites principaux : Évry-Courcouronnes et Palaiseau. Mon activité s'est déroulée majoritairement sur le campus de Palaiseau, au sein des locaux partagés avec Télécom Paris. Ce campus récent offre un cadre de travail moderne et agréable, bénéficiant notamment d'espaces lumineux et d'infrastructures adaptées aux activités d'enseignement et de recherche.

	J'ai également été amené à me rendre ponctuellement sur le campus d'Évry-Courcouronnes, où étaient hébergés les serveurs utilisés pour les expérimentations menées durant le stage. Ces déplacements permettaient notamment la configuration initiale des équipements ainsi que des échanges avec certains membres de l'équipe.

	Je disposais d'un espace de travail sur chacun des deux sites, partagé avec d'autres stagiaires et doctorants. L'environnement de bureau, calme et propice à la concentration, facilitait le travail de recherche et les activités de développement nécessaires au projet.

	\subsection{Organisation du travail}

	Le stage s'inscrivait dans le cadre du projet DECODES (\textit{Optimisation de la Consommation Énergétique des Datacenters par l'Évaluation des DNS}), mené en collaboration avec plusieurs partenaires académiques et industriels, notamment l'AFNIC, IXFrance, l'ENPC et Télécom SudParis.
	...
	\textbf{Méthodologie} \\
	Détails techniques + Diagramme de la configuration.
	...
	\subsection{Consommation par protocole}

	La consommation énergétique est mesurée à l'aide d'un wattmètre (Yoctopuce) avec un taux d'échantillonnage de 100 Hz. Les mesures sont effectuées en envoyant des requêtes au résolveur avec différents QPS (requêtes par seconde). Chaque mesure est échantillonnée 5 fois pour réduire le bruit autant que possible. Chaque protocole est mesuré de la même manière.

	Pour mesurer avec le cache, nous n'envoyons des requêtes que pour un seul nom de domaine afin qu'il soit stocké dans le cache et qu'il n'y ait pas de résolution récursive. Toutes les mesures sont effectuées avec un TTL maximum du cache à 0 seconde, ce qui laisse effectivement le cache inutilisé. Il y a donc un problème majeur pour les mesures de la consommation du résolveur utilisant le cache. Les mesures sans cache utilisent 1 million de noms de domaine. Cette différence entre les expériences n'est pas bonne : il n'y a pas la même standardisation, ce qui signifie que les résultats ne sont pas suffisamment rigoureux.

	Liste des problèmes à résoudre pour que les mesures soient rigoureuses :
	\begin{itemize}
		\item Utiliser le même ensemble de domaines pour les mesures avec et sans cache
		\item Examiner différents types de connexions pour les protocoles (DoT et DoH)
		\item S'assurer que le cache n'est vraiment pas utilisé dans les mesures sans cache et est pleinement utilisé pour les mesures avec cache
	\end{itemize}

	De plus, il serait intéressant d'avoir des mesures pour des QPS plus élevés et avec différentes configurations DNS (taille du cache, durée maximale du TTL du cache, nombre maximal de clients du résolveur, nombre de threads de travail, etc.) et différentes implémentations DNS (PowerDNS, Unbound, Knot DNS, etc.).

	\begin{figure}[H]
	\centering
	\includegraphics[width=0.9\textwidth]{Graphs/combined_energy_consumption.png}
	\caption{Consommation énergétique moyenne pour UDP, DoT et DoH avec et sans utilisation du cache}
	\label{fig:image1}
	\end{figure}

	Il existe une différence claire de consommation énergétique entre l'utilisation avec et sans cache. Les courbes pour l'utilisation sans cache ont une forme de décroissance exponentielle. En observant les courbes d'utilisation du cache, l'ordre est étrange : UDP n'utilise aucune connexion, il devrait donc être en dessous de toutes les autres courbes. Il pourrait donc y avoir un problème ou quelque chose qui n'est pas encore compris.

	---
	\subsection{Paquets envoyés et reçus par le résolveur}

	Une manière de visualiser l'utilisation du résolveur consiste à examiner chaque paquet envoyé et reçu par le résolveur. Sachant que Bind9 utilise UDP pour communiquer avec les DNS autoritatifs, l'examen des paquets UDP est idéal. Ces paquets incluent de nombreux types de requêtes que nous verrons plus tard. Ces nombres de requêtes sont divisés par le QPS résolu par le résolveur.
	...
	---
	\subsection{Densité de la consommation énergétique}
	...
	---
	\subsection{Différence entre cache et non-cache}
	...
	---
	\subsection{Distribution des TTL}
	...
	---
	\subsection{Types de requêtes effectuées par le résolveur}
	...
	---
	\subsection{Prédiction de la consommation énergétique du DNS}
	...
	---
	\subsection{Nouvelle méthodologie de mesure}
